...
void Stage4Reset(DWORD_PTR, DWORD_PTR);
...
void Stage_04::update()
{
	Scene::update();
	StageKeyInput();
}

void Stage_04::finalupdate()
{
	Scene::finalupdate();
}

void Stage_04::render(HDC _dc)
{
	Scene::render(_dc);
}
...
void Stage_04::Enter()
{
	...
	m_pMap->init(m_pPlayer, 5, 6, sPlayerIndex, startIndex);
	...
	//item 
	MakeItem(ITEM_TYPE::SCISSORS, startIndex.x+2,startIndex.y+1);
	MakeItem(ITEM_TYPE::SCISSORS, startIndex.x+1,startIndex.y+2);
	MakeItem(ITEM_TYPE::DOLL, 4, 3);
	MakeItem(ITEM_TYPE::DOLL, 5, 3);
	MakeItem(ITEM_TYPE::DOLL, 3, 4);

	//wall
	m_pMap->SetWall(startIndex.x+1, startIndex.y, POS::RIGHT);
	...
	m_pMap->SetWall(startIndex.x+4, startIndex.y+1, POS::DOWN);
	...
	//ribbon
	m_pMap->SetRibbon(startIndex.x + 3, startIndex.y + 2, POS::RIGHT);
...
void Stage_04::MakeUI() // 추가
{
	...
	pButtonReset->SetClickedCallBack(Stage4Reset, 0, 0);
}
