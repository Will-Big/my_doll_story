...
void Stage7Reset(DWORD_PTR, DWORD_PTR);
...
void Stage_07::update()
{
	Scene::update();
	StageKeyInput();
}

void Stage_07::finalupdate()
{
	Scene::finalupdate();
}

void Stage_07::render(HDC _dc)
{
	Scene::render(_dc);
}
...
void Stage_07::Enter()
{
	...
	//item
	MakeItem(ITEM_TYPE::TAPE, 1, 5);
	MakeItem(ITEM_TYPE::TAPE, 4, 4);
	MakeItem(ITEM_TYPE::DOLL, startIndex.x + 3, startIndex.y + 1);
	MakeItem(ITEM_TYPE::DOLL, startIndex.x + 5, startIndex.y + 1);
	
	// torn tile
	m_pMap->MakeTornOnOff(startIndex.x, startIndex.y);
	m_pMap->MakeTornOnOff(startIndex.x + 1, startIndex.y + 2);
	m_pMap->MakeTornOnOff(startIndex.x + 4, startIndex.y + 1);

	//wall
	m_pMap->SetWall(startIndex.x + 2, startIndex.y + 1, POS::LEFT);
	m_pMap->SetWall(startIndex.x + 2, startIndex.y + 1, POS::DOWN);
	m_pMap->SetWall(startIndex.x + 3, startIndex.y + 1, POS::DOWN);
	m_pMap->SetWall(startIndex.x + 3, startIndex.y + 1, POS::UP);
	m_pMap->SetWall(startIndex.x + 4, startIndex.y + 1, POS::UP);
	m_pMap->SetWall(startIndex.x + 5, startIndex.y + 1, POS::UP);
	m_pMap->SetWall(startIndex.x + 5, startIndex.y + 1, POS::RIGHT);
	m_pMap->SetWall(startIndex.x + 5, startIndex.y + 1, POS::DOWN);
	...
	m_pMap->SetWall(startIndex.x + 2, startIndex.y + 5, POS::LEFT);
	m_pMap->SetWall(startIndex.x + 2, startIndex.y + 5, POS::UP);
	m_pMap->SetWall(startIndex.x + 3, startIndex.y + 5, POS::UP);
	m_pMap->SetWall(startIndex.x + 4, startIndex.y + 5, POS::UP);
	m_pMap->SetWall(startIndex.x + 5, startIndex.y + 5, POS::UP);
	m_pMap->SetWall(startIndex.x + 4, startIndex.y + 4, POS::LEFT);
	m_pMap->SetWall(startIndex.x + 4, startIndex.y + 4, POS::RIGHT);
...
void Stage_07::MakeUI() // 추가
{
	...
	pButtonReset->SetClickedCallBack(Stage7Reset, 0, 0);;
}
