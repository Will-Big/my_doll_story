...
void Stage5Reset(DWORD_PTR, DWORD_PTR);
...
void Stage_05::update()
{
	Scene::update();
	StageKeyInput();
}

void Stage_05::finalupdate()
{
	Scene::finalupdate();
}

void Stage_05::render(HDC _dc)
{
	Scene::render(_dc);
}
...
void Stage_05::Enter()
{
	...
	m_pMap->init(m_pPlayer, 9, 6, sPlayerIndex, startIndex);
	...
	//item
	MakeItem(ITEM_TYPE::COTTON, 1, 2);
	MakeItem(ITEM_TYPE::COTTON, 3, 2);
	MakeItem(ITEM_TYPE::COTTON, 7, 0);
	MakeItem(ITEM_TYPE::COTTON, 7, 2);
	MakeItem(ITEM_TYPE::DOLL, 2, 1);
	MakeItem(ITEM_TYPE::DOLL, 8, 1);
	MakeItem(ITEM_TYPE::DOLL, 8, 2);

	//wall
	m_pMap->SetWall(startIndex.x, startIndex.y + 1, POS::DOWN);
	...
	m_pMap->SetWall(startIndex.x + 1, startIndex.y + 3, POS::LEFT);
	m_pMap->SetWall(startIndex.x + 1, startIndex.y + 3, POS::RIGHT);
	...
	m_pMap->SetWall(startIndex.x + 3, startIndex.y + 4, POS::RIGHT);
	...
	m_pMap->SetWall(startIndex.x + 5, startIndex.y, POS::RIGHT);
	m_pMap->SetWall(startIndex.x + 5, startIndex.y, POS::DOWN);
	m_pMap->SetWall(startIndex.x + 6, startIndex.y + 1, POS::DOWN);
	m_pMap->SetWall(startIndex.x + 6, startIndex.y + 1, POS::RIGHT);
	m_pMap->SetWall(startIndex.x + 6, startIndex.y + 2, POS::LEFT);
	m_pMap->SetWall(startIndex.x + 7, startIndex.y + 1, POS::RIGHT);
	m_pMap->SetWall(startIndex.x + 8, startIndex.y + 2, POS::DOWN);
	m_pMap->SetWall(startIndex.x + 7, startIndex.y + 3, POS::LEFT);
	m_pMap->SetWall(startIndex.x + 7, startIndex.y + 4, POS::LEFT);
	m_pMap->SetWall(startIndex.x + 7, startIndex.y + 4, POS::DOWN);
	m_pMap->SetWall(startIndex.x + 6, startIndex.y + 5, POS::LEFT);
...
void Stage_05::MakeUI() // 추가
{
	...
	pButtonReset->SetClickedCallBack(Stage5Reset, 0, 0);
}
