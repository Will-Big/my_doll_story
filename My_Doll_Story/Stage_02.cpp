...
void Stage2Reset(DWORD_PTR, DWORD_PTR);
...
void Stage_02::finalupdate()
{
	Scene::finalupdate();
}

void Stage_02::render(HDC _dc)
{
	Scene::render(_dc);
}
...
void Stage_02::Enter()
{
	...
	m_pMap->init(m_pPlayer, 4, 5, sPlayerIndex, startIndex);
	...
	//item
	MakeItem(ITEM_TYPE::DOLL, 4, 2);
	MakeItem(ITEM_TYPE::DOLL, 3, 3);
	MakeItem(ITEM_TYPE::DOLL, 4, 4);

	//wall
	m_pMap->SetWall(startIndex.x + 1, startIndex.y, POS::DOWN);
	m_pMap->SetWall(startIndex.x , startIndex.y+1, POS::RIGHT);
	m_pMap->SetWall(startIndex.x+2 , startIndex.y+1, POS::RIGHT);
...
void Stage_02::MakeUI() // 추가
{
	...
	pButtonReset->SetClickedCallBack(Stage2Reset, 0, 0);
}
